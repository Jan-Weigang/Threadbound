...
def add_channels_from_env():
    ...
    if not channels_json:
        logging.error("No discord channels found in environment.")
        return

    try:
        channels_dict = json.loads(channels_json)
    except json.JSONDecodeError as e:
        logging.error("Error parsing channels JSON:", e)
        return
    ...
    for name, discord_channel_id in channels_dict.items():
        if not DiscordChannel.query.filter_by(discord_channel_id=discord_channel_id).first():
            ...
            logging.info(f"Added channel: {name} with ID {discord_channel_id}")
    db.session.commit()
    logging.info("Channels added successfully.")
...
def add_server_from_env():
    ...
    if not discord_server_id or not server_name:
        logging.error("Error: GUILD_ID and SERVER_NAME must be set in environment variables.")
        return
    ...
    if existing_server:
        logging.info(f"Server '{server_name}' with ID '{discord_server_id}' already exists.")
        return
    ...
    db.session.commit()
    logging.info(f"Added server '{server_name}' with ID '{discord_server_id}' to the database.")
